...
int8_t read(struct EXT2DriverRequest request) {
    ...
    while (offset < BLOCK_SIZE) {
        ...
        if (entry->name_len == request.name_len &&
            memcmp(entry_name, request.name, request.name_len) == 0) {
            ...
            for (int i = 0; i < 12 && remaining > 0; i++) {
                ...
                read_blocks(&tmp, file_inode.i_block[i], 1);
                uint32_t to_read = (remaining < BLOCK_SIZE) ? remaining : BLOCK_SIZE;
                memcpy(buf_ptr, tmp.buf, to_read);
            ...
            if (remaining > 0 && file_inode.i_block[12] != 0) {
                ...
                read_blocks(pointers, file_inode.i_block[12], 1);

                for (uint32_t i = 0; i < POINTERS_PER_BLOCK && remaining > 0; i++) {
                    if (pointers[i] == 0) break;

                    read_blocks(&tmp, pointers[i], 1);
                    uint32_t to_read = (remaining < BLOCK_SIZE) ? remaining : BLOCK_SIZE;
                    memcpy(buf_ptr, tmp.buf, to_read);
            ...
            if (remaining > 0 && file_inode.i_block[13] != 0) {
                uint32_t lvl2[POINTERS_PER_BLOCK] = {0};
                read_blocks(lvl2, file_inode.i_block[13], 1);

                for (uint32_t i = 0; i < POINTERS_PER_BLOCK && remaining > 0; i++) {
                    if (lvl2[i] == 0) break;

                    uint32_t lvl1[POINTERS_PER_BLOCK] = {0};
                    read_blocks(lvl1, lvl2[i], 1);

                    for (uint32_t j = 0; j < POINTERS_PER_BLOCK && remaining > 0; j++) {
                        if (lvl1[j] == 0) break;

                        read_blocks(&tmp, lvl1[j], 1);
                        uint32_t to_read = (remaining < BLOCK_SIZE) ? remaining : BLOCK_SIZE;
                        memcpy(buf_ptr, tmp.buf, to_read);
            ...
            if (remaining > 0 && file_inode.i_block[14] != 0) {
                uint32_t lvl3[POINTERS_PER_BLOCK] = {0};
                read_blocks(lvl3, file_inode.i_block[14], 1);

                for (uint32_t i = 0; i < POINTERS_PER_BLOCK && remaining > 0; i++) {
                    if (lvl3[i] == 0) break;

                    uint32_t lvl2[POINTERS_PER_BLOCK] = {0};
                    read_blocks(lvl2, lvl3[i], 1);

                    for (uint32_t j = 0; j < POINTERS_PER_BLOCK && remaining > 0; j++) {
                        if (lvl2[j] == 0) break;

                        uint32_t lvl1[POINTERS_PER_BLOCK] = {0};
                        read_blocks(lvl1, lvl2[j], 1);

                        for (uint32_t k = 0; k < POINTERS_PER_BLOCK && remaining > 0; k++) {
                            if (lvl1[k] == 0) break;

                            read_blocks(&tmp, lvl1[k], 1);
                            uint32_t to_read = (remaining < BLOCK_SIZE) ? remaining : BLOCK_SIZE;
                            memcpy(buf_ptr, tmp.buf, to_read);
